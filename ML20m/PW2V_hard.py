...
train_validation_cut = int(0.8*len(movies_train))
movies_train_train = movies_train[:train_validation_cut]
movies_train_validation = movies_train[train_validation_cut:]
...
def generate_batch_train(batch_size, window_size):
    ...
    batch_index = 0
    while True:
        num_user_movies = len(movies_train_train[user_index])
        for i in range(num_user_movies):
            for tag in movie_tags[movies_train_train[user_index][i]]:
                ...
                for j in range(window_start, window_end):
                    if i != j:
                        ...
                        labels_words[batch_index, 0] = tag_dictionary[
                            random.choice(movie_tags[movies_train_train[user_index][j]])]

                        batch_movies[batch_index] = movie_dictionary[movies_train_train[user_index][i]]
                        labels_movies[batch_index, 0] = movie_dictionary[movies_train_train[user_index][j]]

                        batch_index += 1
                        if batch_index == batch_size:
                            user_index += 1
                            if user_index == len(movies_train_train):
                                user_index = 0
...
def generate_batch_validation(batch_size, window_size):
    # window_size = window_size for products
    global user_index_validation
    # src words (batch) and context words (labels)
    batch_words_validation = np.ndarray(shape=(batch_size), dtype=np.int32)
    labels_words_validation = np.ndarray(shape=(batch_size, 1), dtype=np.int32)

    batch_movies_validation = np.ndarray(shape=(batch_size), dtype=np.int32)
    labels_movies_validation = np.ndarray(shape=(batch_size, 1), dtype=np.int32)
    batch_index = 0
    while True:
        num_user_movies = len(movies_train_validation[user_index_validation])
        for i in range(num_user_movies):
            for tag in movie_tags[movies_train_validation[user_index_validation][i]]:
                window_start = max(i - window_size, 0)
                window_end = min(num_user_movies, i + window_size)
                for j in range(window_start, window_end):
                    if i != j:
                        batch_words_validation[batch_index] = tag_dictionary[tag]
                        labels_words_validation[batch_index, 0] = tag_dictionary[
                            random.choice(movie_tags[movies_train_validation[user_index_validation][j]])]

                        batch_movies_validation[batch_index] = movie_dictionary[movies_train_validation[user_index_validation][i]]
                        labels_movies_validation[batch_index, 0] = movie_dictionary[movies_train_validation[user_index_validation][j]]

                        batch_index += 1
                        if batch_index == batch_size:
                            user_index_validation += 1
                            if user_index_validation == len(movies_train_validation):
                                user_index_validation = 0
                            return batch_words_validation, labels_words_validation, batch_movies_validation, labels_movies_validation
        user_index_validation += 1
        if user_index_validation == len(movies_train_validation):
            user_index_validation = 0
...
with tf.Session(graph=graph) as session:
    ...
    for step in range(num_steps):
        batch_words, labels_words, batch_movies, labels_movies = generate_batch_train(batch_size, window_size)
        ...
        if step % report_every_steps == 0:
            # validation loss
            batch_words_val, labels_words_val, batch_movies_val, labels_movies_val = generate_batch_validation(batch_size, window_size)
            feed_dict = {train_word_inputs: batch_words_val,
            ...
            print(f"Step = {step:>10}   Loss (tag,movie):    train = {average_loss_tag:>8.2f},{average_loss_movie:>8.2f}    validation = {tag_loss_validation:>8.2f}, {movie_loss_validation:>8.2f}")
            average_loss_tag = 0
            average_loss_movie = 0
    ...
    print(f"Eval: precision = {hr:>10.8f} %, CS precision = {hr_cs:>10.8f} %")
